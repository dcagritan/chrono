...
class CH_VEHICLE_API SCMTerrain_Custom : public SCMTerrain {
  public:
    ...
    class CH_VEHICLE_API SoilParametersCallback_Custom {
      public:
        virtual ~SoilParametersCallback_Custom() {}
    ...
    void RegisterSoilParametersCallback(std::shared_ptr<SoilParametersCallback_Custom> cb);
...
class CH_VEHICLE_API SCMContactableData_Custom {
  public:
    SCMContactableData_Custom(double area_ratio,     ///< area fraction with overriden parameters (in [0,1])
                       double Mohr_cohesion,  ///< cohesion for shear failure [Pa]
                       double Mohr_friction,  ///< friction angle for shear failure [degree]
                       double Janosi_shear    ///< shear parameter in Janosi-Hanamoto formula [m]
    );
...
class CH_VEHICLE_API SCMLoader_Custom : public ChLoadContainer {
  ...
  private:
    ...
    SCMTerrain_Custom::NodeInfo GetNodeInfo(const ChVector<>& loc) const;
    ...
    /// If 'all_nodes = true', return modified nodes from the start of simulation.  Otherwise, return only the nodes
    /// modified over the last step.
    std::vector<SCMTerrain_Custom::NodeLevel> GetModifiedNodes(bool all_nodes = false) const;

    // Modify the level of grid nodes from the given list.
    void SetModifiedNodes(const std::vector<SCMTerrain_Custom::NodeLevel>& nodes);
    ...
    std::shared_ptr<SCMTerrain_Custom::SoilParametersCallback_Custom> m_soil_fun;
    ...
    // Mesh coloring mode
    SCMTerrain_Custom::DataPlotType m_plot_type;
    double m_plot_v_min;
    double m_plot_v_max;
