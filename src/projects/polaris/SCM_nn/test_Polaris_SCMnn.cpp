...
// NN model
std::string NN_module_name = "terrain/sph/wrapped_gnn_cpu.pt";

// Initial vehicle position and orientation
ChVector<> initLoc(0, 0, 0.05);
...
class ProxyTire : public RigidTire {
  public:
    ...
    virtual TerrainForce ReportTireForce(ChTerrain* terrain) const override { std::cout<<"ReportTireForceisbeingcalled"<<std::endl; return m_force; }
...
void NNterrain::Synchronize(double time, const DriverInputs& driver_inputs) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (m_verbose) {
            std::cout << "  tire " << i << " force: " << m_tire_forces[i].force << "  position:       " << w_pos[i] << std::endl;
            // std::cout << "  tire " << i << " force: " << m_tire_forces[i].force << " moment: " << m_tire_forces[i].moment << std::endl;
        }
    }
...
void NNterrain::Advance(double step) {
    ...
    for (int i = 0; i < 4; i++) {
        for (size_t j = 0; j < m_num_particles[i]; j++) {
            ...
            // std::cout<<"before p="<<m_wheel_particles[i][j]->GetPos()<<std::endl;
            auto p = m_wheel_particles[i][j]->GetPos()+m_particle_displacements[i][j];
            // std::cout<<"after p="<<m_wheel_particles[i][j]->GetPos()+m_particle_displacements[i][j]<<std::endl;
  ...
  public:
    ...
    virtual void Synchronize(double time) override {
        ...
        if (eff_time < 2)
            ...
            m_steering = 1.0 * std::sin(CH_C_2PI * (eff_time - 2) / 6);
    }
...
int main(int argc, char* argv[]) {
    ...
    while (t < tend) {
        ...
        if (verbose)
            cout << std::fixed << std::setprecision(4) << "t = " << t << "  STB = " << driver_inputs.m_steering << " "
                 << driver_inputs.m_throttle << " " << driver_inputs.m_braking << "  spd = " << vehicle->GetSpeed()
                 << endl;                 
        ...
        // m_wheels[2] = vehicle->GetWheel(1, LEFT);
        // m_wheels[3] = vehicle->GetWheel(1, RIGHT);
        // m_wheels[i]->GetTire())->m_force
        // std::cout<<vehicle->GetWheel(0, LEFT)->GetTire()->GetTireForce()<<std::endl;
        t += step_size;
